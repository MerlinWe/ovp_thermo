...
spring <- dat %>% prep_ovp("Spring", "night") # ok
...
hist(spring$mean_activity_percent)

cor(spring$phase_mean_THI,spring$prev_phase_max_THI)
# [1] 0.4402266
...
AIC(best_fixed_model_HR) #3104.607
...
summary(top_HR1)
AIC(top_HR1)  #3104.724
#hetzelfde met poly() functie
top_HR2 <- nlme::lme(
  fixed = mean_heartrate ~ season_year + phase_mean_THI + poly(day_season,2) + 
  ...
  control = lmeControl(opt = "optim"))
summary(top_HR2)
AIC(top_HR2) # 3104.724

step_modelHR <- stepAIC(top_HR1, scope = list(lower = ~ phase_mean_THI + mean_activity_percent, upper = ~ .),
                        direction = "both")
summary(step_modelHR) #day_season_sq and and season_year are removed.

AIC(step_modelHR)  #3103.69
top_HR<-step_modelHR

best_fixed_model_BT
formula(best_fixed_model_BT)
AIC(best_fixed_model_BT) #-440.062

#I removed the control option because otherwise StepAIC didn't work
top_BT1 <- nlme::lme(
  fixed = mean_BT_smooth ~ season_year+ day_season+day_season_sq + phase_mean_THI +
...
summary(top_BT1)
AIC(top_BT1) #-440.1078
step_modelBT <- stepAIC(top_BT1, scope = list(lower = ~ phase_mean_THI + mean_activity_percent, upper = ~ .),
                        direction = "both") # day_season  + season_year removed
summary(step_modelBT) #.
AIC(step_modelBT) #-443.0661


#day_season in stead of day_season_sq
top_BT2 <- nlme::lme(
  fixed = mean_BT_smooth ~ day_season+ phase_mean_THI +
    mean_activity_percent + prev_phase_mean_THI,
  data = spring,
  random = ~1 + day_season | ID_phase, 
  correlation = corGaus(form = ~ day_season | ID_phase, nugget = TRUE), 
...
summary(top_BT2)
AIC(top_BT2) #-440.3424 much higher so i will use day_season_sq
...
top_BT_REML <- nlme::lme(
  ...
  correlation = corGaus(form = ~ day_season | ID_phase, nugget = TRUE), 
  method = "REML")
AIC(top_BT_REML) #-382.2488


best_fixed_model_ACT
formula(best_fixed_model_ACT)
AIC(best_fixed_model_ACT) #3089.158
...
top_HR_REML <- nlme::lme(
  fixed = mean_heartrate ~  phase_mean_THI + day_season + 
    mean_activity_percent ,
  data = spring,
  random = ~1+day_season| ID_phase, 
  correlation = corGaus(form = ~ day_season | ID_phase, nugget = TRUE), 

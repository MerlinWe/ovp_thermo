...
# Get winter night subset 
winter <- dat %>% prep_ovp("Winter", "night") # ok
...
#stepAIC did not work for HR, so I will do it manually
# Refit top models using the transformed quadratic terms

# check top models 
best_fixed_model_HR
formula(best_fixed_model_HR)
AIC(best_fixed_model_HR) #2338.953
summary(best_fixed_model_HR)

top_HR1 <- nlme::lme(
  fixed = mean_heartrate ~ season_year + phase_mean_THI +
    day_season+ day_season_sq+ weight+ mean_activity_percent+mean_activity_percent_sq,
  data = winter,
...
top_HR2 <- nlme::lme(
  ...
  control = lmeControl(opt = "optim"))
summary(top_HR2)
AIC(top_HR2) #2340.535 1,5 pts higher so i will not remove mean_activity_percent_sq

#does the model improve if i remove the day_season_sq  ?

top_HR3 <- nlme::lme(
  fixed = mean_heartrate ~ season_year + phase_mean_THI +
    day_season+ weight+ mean_activity_percent+mean_activity_percent_sq,
  data = winter,
  random = ~1+phase_mean_THI| ID_phase, 
  correlation = corGaus(form = ~ day_season | ID_phase, nugget = TRUE), 
  method = "ML", 
  ...
  control = lmeControl(opt = "optim"))
summary(top_HR3)
AIC(top_HR3)  #2342.31 (3.2 pts higher so i will not remove it)
...
AIC(best_fixed_model_BT) #-280.1272
summary(best_fixed_model_BT)
...
summary(top_BT1)
AIC(top_BT1) # -280.1272

step_modelBT <- stepAIC(top_BT1, scope = list(lower = ~ phase_mean_THI + mean_activity_percent, upper = ~ .),
                        direction = "both")
summary(step_modelBT) #day_season_sq+ mean_activity_percent_sq +season_year are removed.
top_BT<-step_modelBT

# check top models 
best_fixed_model_ACT
formula(best_fixed_model_ACT)
AIC(best_fixed_model_ACT) #2534.55
summary(best_fixed_model_ACT)

top_ACT1 <- nlme::lme(
  fixed = mean_activity_percent ~ season_year+phase_mean_THI + weight,
  data = winter,
  random = ~1 | ID_phase, 
  correlation = corGaus(form = ~ day_season | ID_phase, nugget = TRUE), 
  method = "ML", 
  ...
  control = lmeControl(opt = "optim"))
summary(top_ACT1)
AIC(top_ACT1) #2534.55

step_modelACT <- stepAIC(top_ACT1, scope = list(lower = ~ phase_mean_THI, upper = ~ .),
                         direction = "both")
summary(step_modelACT) #removed season_year
AIC(step_modelACT) #2532.889 lower AIC!

#remove weight? (N.S. p=0.1376)
...
top_ACT2 <- nlme::lme(
  ...
  control = lmeControl(opt = "optim"))
summary(top_ACT2)
AIC(top_ACT2) #2533.198 0.4 pts higher so i will remove weight
...
sd_mean_activity_percent_sq <- sd(winter$mean_activity_percent_sq, na.rm = TRUE)
sd_mean_heartrate <- sd(winter$mean_heartrate, na.rm = TRUE)

# Extract unstandardized estimates from model output
beta_phase_mean_THI <- 0.047
beta_day_season <- -0.3116
beta_day_season_sq <- 0.0016
beta_weight <- -0.0968
beta_mean_activity_percent <- 0.5232
beta_mean_activity_percent_sq<- -0.0105

# Compute standardized estimates
std_beta_phase_mean_THI <- beta_phase_mean_THI * (sd_phase_mean_THI / sd_mean_heartrate)
std_day_season <- beta_day_season * (sd_day_season / sd_mean_heartrate)
std_day_season_sq <- beta_day_season_sq * (sd_day_season_sq / sd_mean_heartrate)
...
std_beta_mean_activity_percent_sq <- beta_mean_activity_percent_sq * (sd_mean_activity_percent_sq / sd_mean_heartrate)
...
std_beta_mean_activity_percent_sq
